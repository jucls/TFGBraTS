	...
	\subsection{Métodos que se enfocan en la arquitectura}
		...
		\subsubsection{Diseño de bloques especializados}
			...
			\begin{enumerate}
				...
				\begin{figure}[H]
					\centering
					\includegraphics[width=0.45\linewidth]{imagenes/comparisonsinglemultipleCNN.png}
		...
		\end{enumerate}
			...
			Funciones de pérdida estándar en este problema incluyen categorical cross-entropy, cross-entropy y dice loss $D_{L}$. Esta última siendo muy similar a optimizar una de las métricas que se usan en el problema.
			
			$$ D_{L} =  1 - D_{C} $$
			$$ D_{C}\ es\ el\ coeficiente\ de\ similaridad\ dice\ $$
